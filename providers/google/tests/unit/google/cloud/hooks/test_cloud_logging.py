...
class TestCloudLoggingHook:
    ...
    def test_create_sink(self, mock_config_client, cloud_logging_hook):
        ...
        cloud_logging_hook.create_sink(sink=sink, project_id=PROJECT_ID, unique_writer_identity= UNQUE_WRITER_IDENTITIY)
    ...
    def test_update_sink_success(self, mock_config_client, cloud_logging_hook):
        ...
        hook.update_sink(sink_name=SINK_NAME,sink=sink_config,
            update_mask = update_mask,
            unique_writer_identity= UNQUE_WRITER_IDENTITIY, project_id=PROJECT_ID)
    ...
    def test_create_sink_dict_input(self, mock_config_client, cloud_logging_hook):
        ...
        expected_request = CreateSinkRequest(parent=f"projects/{PROJECT_ID}", sink=expected_sink, unique_writer_identity= UNQUE_WRITER_IDENTITIY)

        cloud_logging_hook._client = mock_config_client.return_value
        cloud_logging_hook.create_sink(sink=sink_config,unique_writer_identity= UNQUE_WRITER_IDENTITIY, project_id=PROJECT_ID)
    ...
    def test_update_sink_failure(self, mock_config_client, cloud_logging_hook):
        ...
        with pytest.raises(Exception, match="Permission denied"):
            cloud_logging_hook.update_sink(sink_name=SINK_NAME, sink=updated_sink,unique_writer_identity=UNQUE_WRITER_IDENTITIY, project_id=PROJECT_ID)
